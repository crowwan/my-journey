...
5. **환율 계산기 배치**: BudgetTab 도넛 차트 아래 + 예산 항목 리스트 위 (인라인 위젯). 예산 확인하면서 바로 환산 가능. 각 항목의 원화 환산도 실시간 환율로 업데이트
...
**목표**: 예산 탭(BudgetTab)에 실시간 환율 계산기 위젯 통합 + 각 항목 원화 환산 실시간화

**UI 배치**: 도넛 차트 아래, 예산 항목 리스트 위에 인라인 위젯
```
[도넛 차트 + 합계]
[━━━ 환율 계산기 ━━━━━━━━━━━━━━━━━━]
│  10,000  KRW  ⇄  1,098  JPY     │
│  1 JPY = 9.11 KRW · 6시간 전 기준  │
[━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━]
[예산 항목 리스트 (각 항목에 실시간 원화 환산)]
```
...
- [ ] 3-2. CurrencyConverter 컴포넌트 생성 - 파일: `app/src/components/viewer/CurrencyConverter.tsx` - 규모: L
  - **위치**: BudgetTab 읽기 모드, 도넛 차트 아래 / 예산 항목 리스트 위
  - 두 개의 입력 필드: KRW / 현지 통화 (한쪽 입력 시 다른 쪽 자동 계산)
  - 스왑 버튼 (⇄) 중앙 배치
  - 현재 환율 표시 ("1 JPY = 9.11 KRW")
  - 마지막 업데이트 시각 표시 ("6시간 전 기준")
  - 통화 코드: Trip의 `budget.currency` 사용 (이미 있음)
  - 로딩 스켈레톤 / 에러 메시지 ("환율 정보를 불러올 수 없습니다")
  - 국내 여행(KRW)이면 숨김
  - 디자인: 기존 카드 스타일 (`bg-surface border border-border-light rounded-xl p-5 shadow-sm`)

- [ ] 3-3. BudgetTab에 환율 계산기 통합 - 파일: `app/src/components/viewer/tabs/BudgetTab.tsx` - 규모: S
  - BudgetReadSection 내부, DonutChart 아래에 CurrencyConverter 배치
  - `budget.currency`가 'KRW'가 아닐 때만 표시
  - 실시간 환율로 각 항목의 원화 환산 업데이트 (기존 exchangeRate 대신)
...
- [ ] 3-4. 목적지 -> 통화 코드 매핑 유틸 - 파일: `app/src/lib/currency-utils.ts` - 규모: S
  ...
  - 국내 여행은 null 반환
  - 참고: Trip에 이미 `budget.currency`가 있으므로, 이 유틸은 currency가 없는 레거시 데이터 폴백용
