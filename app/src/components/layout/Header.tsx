...
export function Header({ title = 'My Journey', showBack = false }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-border px-4 py-4 pt-[calc(0.75rem+env(safe-area-inset-top))] flex items-center gap-3 shadow-none">
      {showBack && (
        <button
          onClick={() => router.back()}
